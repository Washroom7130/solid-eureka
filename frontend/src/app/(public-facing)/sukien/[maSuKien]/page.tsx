...
export default function EventDetailPage() {
  ...
  useEffect(() => {
    ...
    fetch(`http://backend:5555/api/sukien/get/${maSuKien}`, {
      credentials: 'include',
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  useEffect(() => {
    ...
    fetch(`http://backend:5555/api/danhgia/sukien/${maSuKien}/get/all`, {
      credentials: 'include',
    })
      .then((res) => res.json())
  ...
  const handleSubmitReview = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`http://backend:5555/api/danhgia/${event.maSuKien}/add`, {
        method: 'POST',
        headers: {
        ...
        <div className="event-detail-wrapper">
          <div className="event-image">
            <img id="event-img" src={event.anhSuKien === null ? 
              ...
              `http://backend:5555/api/sukien/get${event.anhSuKien}`} alt="Ảnh sự kiện" />
          </div>
