...
export default function OrdersPage() {
  ...
  const fetchOrders = async (page: number) => {
    try {
      const res = await fetch(`http://localhost:55555/api/hoadon/get/all?page=${page}&size=5`, {
        credentials: 'include',
      });
  ...
  const fetchCauHoi = async (maSuKien: string, maHoaDon: string) => {
    try {
      const res = await fetch(`http://localhost:55555/api/cauhoi/get/sukien/${maSuKien}`, {
        credentials: 'include',
      });
