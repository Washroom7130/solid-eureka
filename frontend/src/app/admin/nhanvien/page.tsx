...
export default function NhanVienPage() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch(`http://localhost:55555/api/admin/nhanvien/get/all?page=${page}&size=${pageSize}&search=${encodeURIComponent(search)}`, {
        credentials: 'include',
      });
      if (res.ok) {
  ...
  const handleDetailClick = async (maId: number) => {
    ...
    try {
      const res = await fetch(`http://localhost:55555/api/admin/nhanvien/get/${maId}`, {
        credentials: 'include'
      });
      const data = await res.json();
      if (res.ok) {
  ...
  const handleAddNhanVien = async () => {
    ...
    try {
      const res = await fetch('http://localhost:55555/api/admin/nhanvien/add', {
        method: 'POST',
        credentials: 'include',
                ...
                onClick={async () => {
                  ...
                  const endpoint =
                    selectedStatus === 'Hoạt Động'
                      ? `http://localhost:55555/api/admin/nhanvien/deactivate/${selectedIdForStatusChange}`
                      : `http://localhost:55555/api/admin/nhanvien/activate/${selectedIdForStatusChange}`;
