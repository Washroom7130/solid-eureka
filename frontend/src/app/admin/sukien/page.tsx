...
export default function SuKienTablePage() {
  ...
  const fetchData = async () => {
    const url = new URL('http://backend:5555/api/sukien/get/all')
  ...
  useEffect(() => {
    if (addModalOpen || editModalOpen) {
      fetch('http://backend:5555/api/danhmucsukien/get/all?size=100', {
        credentials: 'include',
      })
        .then((res) => res.json())
        .then((data) => {
  ...
  const onSubmitAddForm = async () => {
    ...
    try {
      ...
      const res = await fetch('http://backend:5555/api/sukien/add', {
        method: 'POST',
        body: formData,
        credentials: 'include',
      });
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      const res = await fetch(`http://backend:5555/api/sukien/update/${editForm.maSuKien}`, {
        method: 'POST',
        body: formData,
        credentials: 'include',
      });
  ...
  className="edit-btn"
  onClick={() => {
    fetch(`http://backend:5555/api/sukien/get/${sk.maSuKien}`, { credentials: 'include' })
      .then((res) => res.json())
      .then((data) => {
        setEditForm({
        ...
        <div style={{ flex: '1', minWidth: '200px' }}>
          ...
          >
            {editImage ? (
              ...
              <img
                src={`http://backend:5555/api/sukien/get${editForm.anhSuKien}`}
                alt="Hiện tại"
                style={{ maxWidth: '100%', maxHeight: '100%' }}
              />
            ) : (
    ...
    <div>
      ...
      <div className="modal-buttons" style={{ marginTop: '20px' }}>
        <button
          ...
          onClick={async () => {
            try {
              const res = await fetch(`http://backend:5555/api/sukien/cancel/${selectedCancel.maSuKien}`, {
                method: 'PUT',
                credentials: 'include',
              });
